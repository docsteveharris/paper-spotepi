...
def run_fchk(col, chk):
    ...
    if chk_type in ['legal_alias', 'legal_vals']:
...
for field, col in cols_dict.items():
    ...
    if checks:
        for chk in fspec['checks']:
            ...
            # FIXME: 2014-09-16 - using aliases seems to defeat the purpose: need to have converted already?
...
for rowkey, row in rows_dict.items():
    for index, item in enumerate(row):
        ...
        if item['imposs_fchks'] + item['unusual_fchks'] > 0:
            item['validated'] = None
