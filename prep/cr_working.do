...
*  =======================================
* GenericSetupSteveHarris paper-spotearly cr_working, logon
...
===
140624
- drop Northampton b/c 2 critical care units
150629
- switched to using a symlinked version of the data from spot-epi
- added in project paths code
...
clear
* NOTE: 2014-11-25 - [ ] commented out SQL query for now
...
* CHANGED: 2015-11-06 - [ ] drop Northampton here (so discuss 48 sites)
count if include == 1 & inlist(icode, "84s") & include == 1 // HDU not in CMP
replace include = 0 if inlist(icode, "84s") & include == 1 // HDU not in CMP
...
count if include == 1 ///
	& (inlist(v_arrest, 2, 3) | inlist(v_disposal, 7)) ///
	& exclude1 == 0
...
count if include == 1 ///
	& (elgreport_heads == 0 | elgreport_tails == 0) ///
	& exclude1 == 0

* Drop exclusions for rxlimits and theatre
* CHANGED: 2014-06-04 - changed from paper-spotepi
* Admissions via theatre remain in the analysis
* count if include == 1 & loca == 1 & exclude1 == 0
* Patients with treatment limitation orders
...
* CHANGED: 2014-06-04 - changed from paper-spotepi
* Specific excludes for early analysis
* --
/*
...
*  ====================================================
*  = Fourth exclusion - errors in timing and survival =
*  ====================================================
gen exclude4 = 0
label var exclude3 "Exclude - irreconcilable errors"
...
replace exclude4 = 1 if dead_icu == 1 & dead == 0 ///
	& exclude1 == 0 & exclude2 == 0 & exclude3 == 0
...
* Patients dropped because of errors in CRF that cannot be reconciled
