...
cap program drop count_sepsis
program count_sepsis
	// syntax varname , newslevel(integer 3)
	syntax varname [, category(integer 3)]
	...
	if $category == 1 {
		local ttitle "Severe sepsis"
		global catname "`ttitle'"
	}

	if $category == 2 {
		local ttitle "Septic shock"
		global catname "`ttitle'"
	}
	...
	tw hist vperweek, ///
		...
		yscale(noextend) ylab(0(100)600, nogrid) ///
		ytitle("Study-site-weeks") ///
		xtitle("New bedside assessments", margin(medium)) ///
		title("`ttitle'")

	graph rename vperweek_sepsis$category, replace

	// quick inspect the distribution of incidence
	graph export ../logs/vperweek_sepsis$category.pdf, replace
...
tab sepsis2001
label list sepsis2001
cap drop sepsis_cat
gen sepsis_cat = .
replace sepsis_cat = 1 if sepsis2001 == 3
replace sepsis_cat = 2 if inlist(sepsis2001, 4,5,6)
tab sepsis_cat
...
forvalues i = 1/2 {
	use `working1', clear
	count_sepsis sepsis_cat , category(`i')
	est store news`i'
	/*
	...
	*/
	...
	if `first_run' {
		use `temp1', clear
		save ../data/count_sepsis_cat_estimates.dta, replace
	}
	else {
		use ../data/count_sepsis_cat_estimates, clear
		append using `temp1'
		save ../data/count_sepsis_cat_estimates.dta, replace
	}
	local first_run 0
...
cap graph combine vperweek_sepsis1 perweek_sepsis2 , rows(1) ///
	xcommon ycommon
save ../data/count_sepsis_cat_estimates.dta, replace
...
forvalues i = 1/2 {
	use ../data/count_sepsis_cat_estimates.dta, clear
	* CHANGED: 2013-02-04 - comment out one to produce table
	keep if idnum == `i'
	if `i' == 1 local table_name incidence_sepsis_severe
	if `i' == 2 local table_name incidence_septic_shock
	//keep if idnum == 2 // septic shock
	// keep if idnum == 1 // severe sepsis
